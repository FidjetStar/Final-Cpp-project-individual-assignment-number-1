...
void PlayGame(Player& p1, Player& p2) {
    ...
    while (!p1.cards.empty() && !p2.cards.empty() && round <= 1000) {
        ...
        while (c1 == c2 && !p1.cards.empty() && !p2.cards.empty()) {
            std::cout << "СПОР! Карты равны(игра продолжится пока у одного из игроков не откроется карта более высокого ранга).\n";
            ...
            if (p1.cards.empty() || p2.cards.empty())
                if (p1.cards.empty()) {
                    std::cout << "У первого игрока закончились карты.\n";
                    break;
                }
                else {
                    std::cout << "У второго игрока закончились карты.\n";
                    break;
                }
        ...
        if (c1 > c2) {
            ...
            log.push_back(entry + " ? Победил Игрок 1");
        }
        else if (c2 > c1) {
            std::cout << "Победил Игрок 2 в этом раунде!\n";
            for (int card : table)
                p2.cards.push_back(card);
            log.push_back(entry + " ? Победил Игрок 2");
        }
        else {
            log.push_back(entry + " ? Ничья");
        }
        round++;
    }
    std::cout << "\n===== ИГРА ОКОНЧЕНА =====\n";
